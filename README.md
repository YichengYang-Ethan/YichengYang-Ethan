...
```
prediction-market-pricing (Theory)
    ├── oracle3 (On-chain execution — 8 arb strategies from formal invariants)
    └── coinjure (Strategy lifecycle — discovery → backtest → deploy → retire)

Market-Bubble-Index (Risk signal)
    └── clawdfolio (Bubble Score → covered call timing → +3.0% annualized alpha)
```
